  ...
  return (
    <div className={`mb-16 ${isCenter ? 'text-center' : ''}`}>
      <div className={`flex items-center gap-3 mb-6 reveal ${isCenter ? 'justify-center' : ''}`}>
        ...
        <span className="text-cream/45 text-xs font-semibold tracking-[0.2em] uppercase">{label}</span>
      </div>
      <h2 className={`display-lg font-display font-bold text-cream reveal ${isCenter ? 'mx-auto' : ''}`}>
        {title}
      </h2>
      {subtitle && (
        <p className={`text-cream/40 text-lg leading-relaxed mt-6 reveal ${isCenter ? 'max-w-xl mx-auto' : 'max-w-xl'}`}>
          {subtitle}
        </p>
      )}
    </div>
